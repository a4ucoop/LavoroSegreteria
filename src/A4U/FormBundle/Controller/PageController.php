...
// -------------------------------PORTE APERTE ESTATE-------------------------------------
    ...
    public function reportPAEAction()
    {
       ...
       foreach ($subscribed as $key) {
        array_push($fcsubscribed, strtolower($key['COD_FIS']));
       }
       ...
       foreach ($Users as $iscrittoForms) {
           ...
           if ($pos!==False) {
                ...
                $dati_iscritto->CFSTUDENTE=$subscribed[$pos]['COD_FIS'];
                ...
                $dati_iscritto->anno=$subscribed[$pos]['AA_ID'];
    ...
    public function reportStageAction()
    {
       ...
       foreach ($Users as $iscrittoForms) {
           ...
           if ($posFC!==False || $posNC!==False) {
                ...
                //  vero matte?
                $dati_iscritto = new bothIscritti();
                $dati_iscritto->CFSTUDENTE=$subscribed[($posFC!==False) ? $posFC : $posNC]['COD_FIS'];
                ...
                $dati_iscritto->anno=$subscribed[($posFC!==False) ? $posFC : $posNC]['AA_ID'];
    ...
    public function matchEsse3Action($repository)
    {
       ...
       $conn = OCILogon("esse3_unicam_prod_read","r34d3ss33",$db);
        ...
        $query="SELECT
            ...
            SA.AA_ID,
            SC.NOMECDS,
            SUM(ES.VOTO)/COUNT(ES.VOTO) AS MEDIACERTIFICATA,
            SUM(ES.CREDITI) AS CFUCERTIFICATI
            FROM
            ((V11_STAT_ESAMI_SOSTENUTI ES RIGHT JOIN
            V_STAT_ANAGRAFICA SA ON
              ES.COD_FIS=SA.COD_FIS) INNER JOIN V11_ERGO_STATO_CARRIERA SC ON (SA.AA_ID=SC.AAID AND ES.CDS_DESC=SC.NOMECDS AND SA.COD_FIS=SC.CFSTUDENTE))
            WHERE
                ";
        ...
        foreach ($iscritti as $key) {
            if($key->getFiscalcode() !== null){
                $append="lower(SA.COD_FIS)='";
                $query=$query . $append . str_replace("'", "''", strtolower($key->getFiscalcode())) . "' OR ";
            } else {
                $append="(lower(SA.NOME)='";
                $query=$query . $append . str_replace("'", "''", strtolower($key->getName())) . "' AND ";
                $append="lower(SA.COGNOME)='";
                $query=$query . $append . str_replace("'", "''", strtolower($key->getSurname())) . "') OR ";
            }
        }
       ...
       while ($row = oci_fetch_array($statement, OCI_ASSOC+OCI_RETURN_NULLS)) {
           array_push($esse3_data,$row);
           foreach (array_keys($row) as $into)
               echo($into); 
       } 
